...
public class TDS_Throwable : PunBehaviour
{
	...
	*/
    ...
    [SerializeField] protected bool wasThrowByPlayer = false;
    ...
    public Dictionary<int, int> CheckHit()
    {
        ...
        {
            if ((wasThrowByPlayer && !(_target = _collider.GetComponent<TDS_Player>())) || (_target = _collider.GetComponent<TDS_DamageableElement>()))
            {
    ...
    public virtual void Grab(TDS_Player _graber)
    {
        if (!isThrowable || isReadyToThrow) return;
    ...
    public virtual void Throw(FacingSide _facingSide)
    {
        ...
        trajectoryPositions = new Vector3[] { transform.position + (_orientation * .5f) + (Vector3.up * .5f) + 
           transform.position + (_orientation * 1f) - (Vector3.up * .5f) + 
           transform.position + (_orientation * 1.5f) - (Vector3.up * transform.position.y) };
    ...
    private void OnGUI()
    {
        if (isReadyToThrow)
        {
            ...
            Handles.DrawWireCube(transform.position + (Vector3.up * 2), new Vector3(.1f, .1f, .1f));
        }
        ...
        Handles.DrawDottedLines(trajectoryPositions, 1 / trajectoryPositions.Length);
